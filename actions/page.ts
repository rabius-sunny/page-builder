...
export async function updatePageSectionsBySlug(slug: string, sections: PageSection[]) {
  try {
    await connectToDatabase()

    const updatedPage = await Pages.findOneAndUpdate({ slug }, { sections }, { new: true })
    ...
    console.error('Error updating page sections by slug:', error)
    return { success: false, error: 'Failed to update page sections' }
  }
}
