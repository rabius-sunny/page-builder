...
import { getPageBySlug, updatePageSectionsBySlug } from '@/actions/page'
import { useState, useEffect } from 'react'
import { toast } from 'sonner'
import { useQueryState } from 'nuqs'

export function useCurrentPage() {
  const [pageSlug, setPageSlug] = useQueryState('page', {
    defaultValue: '',
    shallow: false
  })
  ...
  // Load page when pageSlug changes
  useEffect(() => {
    if (pageSlug) {
      loadPageData(pageSlug)
    } else {
      setCurrentPage(null)
      setSections([])
    }
  ...
  const loadPageData = async (slug: string) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // If page not found, clear the URL param
        setPageSlug('')
  ...
  const loadPage = async (newPageSlug: string) => {
    setPageSlug(newPageSlug)
    return null // The actual loading will happen in useEffect
  }
  ...
  const savePage = async () => {
    ...
    setSaving(true)
    try {
      const result = await updatePageSectionsBySlug(currentPage.slug, sections)
      if (result.success) {
  ...
  // Update setCurrentPage to also update URL when called directly
  const updateCurrentPage = (page: CustomPage | null) => {
    setCurrentPage(page)
    if (page && page.slug && page.slug !== pageSlug) {
